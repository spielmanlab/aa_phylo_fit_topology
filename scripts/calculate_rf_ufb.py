...
if type ==  "pandit":
    ...
    for pandit in pandits:
        ...
        m1_tree = dendropy.Tree.get_from_path(inferencepath + pandit + "_m1_inferredtree.contree" , "newick", taxon_namespace = ts, rooting='force-unrooted')
        m1_tree.encode_bipartitions()
    
        iqfiles = [x for x in iqfiles_all if x.startswith(pandit)]

        for iqfile in iqfiles:
            fileinfo = iqfile.split("_")
            model = fileinfo[1]
            treefile = iqfile.replace(".iqtree", ".contree")
            ...
            bp_prefix = pandit + "," + model
            outstring_boot += compare_ufb(bp_prefix, inftree, m1_tree)

            fit_string = obtain_fit_info(inferencepath + iqfile, fitinfo_order)
            part = bp_prefix + "," + fit_string + "," + rf_m1 + "," + tl  + "\n"
