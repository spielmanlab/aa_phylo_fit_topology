---
title: "Linear models presented in manuscript"
...
SIG: m5 diffs from all, most 0.0617 for m1
     GTR diffs jc,m1,m5
     JC diff m4,m5,gtr
...
glht(fit, linfct=mcp(model_levels='Tukey')) %>% 
  ...
  style_kable_sig()
```
SIG: m5 diffs from all except jc, most 0.016 for m1 AND gtr but opp signs
     GTR diffs jc,m5
     JC diff m1,m2,m3



### Model 4: Entropy among simulations

```{r model-4, warning=FALSE, message=FALSE}


## Fit the model
lm(total_entropy ~ name_levels+tree_levels, data = entropy) %>% 
  broom::tidy() %>% 
  mutate(sig = p.value <= SIG.ALPHA) %>%
  style_kable_sig()
