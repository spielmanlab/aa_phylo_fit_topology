...
empirical_rf_fit %>% 
  ...
  mutate(model_levels = factor(model, levels=model_levels, labels = model_labels),
    ...
    ggplot(aes(x = optimizedtruetree, y = inferredtree)) + 
       ...
       theme(axis.text = element_text(size=8))-> empirical_tl_inf_vs_optim
ggsave(paste0(figure_directory, "empirical_tl_inf_vs_optim.pdf"), empirical_tl_inf_vs_optim, height = 5, width=7)
...
pandit_rf_fit %>% 
    ...
    mutate(tl_norm = treelength / m1_treelength,
           model_levels = factor(model, levels=model_levels_noq1, labels = model_labels_noq1)) %>%
...
#  & name & tree & model & n \\ 
#   \hline
# 1 & HIV & opisthokonta & q5 &   3 \\ 
#   2 & HIV & ruhfel & q5 &   1 \\ 
#   3 & NP & opisthokonta & q5 &   1 \\ 
